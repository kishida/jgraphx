...
public class mxStencil implements mxIShape {
    ...
    protected void drawElement(mxGraphicsCanvas2D canvas, mxCellState state,
        ...
        switch (name) {
            ...
            case "rect" -> canvas.rect(x0 + getDouble(node, "x") * sx,
            ...
            case "ellipse" -> canvas.ellipse(x0 + getDouble(node, "x") * sx,
                        y0 + getDouble(node, "y") * sy, getDouble(node, "w") * sx,
                        getDouble(node, "h") * sy);
            case "image" -> {
            ...
            case "fillstroke" -> canvas.fillAndStroke();
            case "fill" -> canvas.fill();
            case "stroke" -> canvas.stroke();
            case "strokewidth" -> {
            ...
            case "dashpattern" -> {
                ...
                if (value != null) {
                    ...
                    for (String tmp1 : tmp) {
                        if (tmp1.length() > 0) {
                            pat.append(Double.parseDouble(tmp1) * minScale);
                            pat.append(" ");
                        }
                    }
            ...
            case "strokecolor" -> canvas.setStrokeColor(node.getAttribute("color"));
            case "linecap" -> canvas.setLineCap(node.getAttribute("cap"));
            case "linejoin" -> canvas.setLineJoin(node.getAttribute("join"));
            case "miterlimit" -> canvas.setMiterLimit(getDouble(node, "limit"));
            case "fillcolor" -> canvas.setFillColor(node.getAttribute("color"));
            case "fontcolor" -> canvas.setFontColor(node.getAttribute("color"));
            case "fontstyle" -> canvas.setFontStyle(getInt(node, "style", 0));
            case "fontfamily" -> canvas.setFontFamily(node.getAttribute("family"));
            case "fontsize" -> canvas.setFontSize(getDouble(node, "size") * minScale);
            default -> {
            }
        }
    }
