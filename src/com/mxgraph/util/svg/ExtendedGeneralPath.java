...
public class ExtendedGeneralPath implements Shape, Cloneable {
    ...
    /**
     * Constructs a new <code>ExtendedGeneralPath</code> with the specified
    ...
    /**
     * Constructs a new <code>ExtendedGeneralPath</code> object with the
    ...
    /**
     * Constructs a new <code>ExtendedGeneralPath</code> object from an
     * arbitrary <code>Shape</code> object.
     */
    ...
    /**
     * Checks if previous command was a moveto command, skipping a close command
     * (if present).
     */
    protected void checkMoveTo() {
        ...
        switch (types[numSeg - 1]) {
            ...
            case PathIterator.SEG_MOVETO -> path.moveTo(values[numVals - 2], values[numVals - 1]);
    ...
    public void append(PathIterator pi, boolean connect) {
        ...
        while (!pi.isDone()) {
            ...
            switch (type) {
                case PathIterator.SEG_CLOSE -> closePath();
                case PathIterator.SEG_MOVETO -> moveTo((float) vals[0], (float) vals[1]);
                case PathIterator.SEG_LINETO -> lineTo((float) vals[0], (float) vals[1]);
                case PathIterator.SEG_QUADTO -> quadTo((float) vals[0], (float) vals[1], (float) vals[2],
                            (float) vals[3]);
                case PathIterator.SEG_CUBICTO -> curveTo((float) vals[0], (float) vals[1], (float) vals[2],
    ...
    public void append(ExtendedPathIterator epi, boolean connect) {
        ...
        while (!epi.isDone()) {
            ...
            switch (type) {
                case PathIterator.SEG_CLOSE -> closePath();
                case PathIterator.SEG_MOVETO -> moveTo(vals[0], vals[1]);
                case PathIterator.SEG_LINETO -> lineTo(vals[0], vals[1]);
                case PathIterator.SEG_QUADTO -> quadTo(vals[0], vals[1], vals[2], vals[3]);
                case PathIterator.SEG_CUBICTO -> curveTo(vals[0], vals[1], vals[2], vals[3], vals[4],
                            vals[5]);
                case ExtendedPathIterator.SEG_ARCTO -> arcTo(vals[0], vals[1], vals[2], (vals[3] != 0),
    ...
    class EPI implements ExtendedPathIterator {
        ...
        public int currentSegment(float[] coords) {
            ...
            switch (ret) {
                ...
                case SEG_QUADTO -> System.arraycopy(values, valsIdx, coords, 0, 4);
                case SEG_CUBICTO -> System.arraycopy(values, valsIdx, coords, 0, 6);
        ...
        public void next() {
            ...
            switch (type) {
                ...
                case SEG_MOVETO, SEG_LINETO -> // fallthrough is intended
                    valsIdx += 2;
                case SEG_QUADTO -> valsIdx += 4;
                case SEG_CUBICTO -> valsIdx += 6;
                case SEG_ARCTO -> valsIdx += 7;
            }
     ...
     * Must be called even for numValues = 0, because it is also used for
