...
public class mxCurve {
     ...
     * a line, parallel with the curve. If the distance or curve is invalid,
     ...
     *
     * @param index the curve index specifying the curve to analyse
     * @param rect the whose boundary is to be tested for intersection with this
     * curve
     ...
     * @param buffer the amount by which the rectangle is to be moved, beyond
     ...
     * curve
     * @return the point at which this curve intersects the boundary of the
     * given rectangle, if it does so. If it does not intersect, -1 is returned
     */
     ...
     * 1), y is the orthogonal offset from the closest segment on the curve and
     * (width, height) is an additional Cartesian offset applied after the other
     * calculations
     */
    ...
    protected void createCoreCurve() {
        ...
        List<mxPoint> coreCurve = new ArrayList<>();
        List<Double> coreIntervals = new ArrayList<>();
    ...
    protected void createLabelCurve() {
        ...
        List<mxPoint> labelCurvePoints = new ArrayList<>();
            ...
            if (newPoints.size() == guidePoints.size() && newPoints.size() > 1
                ...
                if (trivialTranslation) {
                    ...
                    for (mxPoint[] curve : curves) {
                        for (mxPoint curve1 : curve) {
                            curve1.setX(curve1.getX() + transX);
                            curve1.setY(curve1.getY() + transY);
                        }
                    }
