...
public class mxGraphGenerator {
	...
	public void getKnightGraph(mxAnalysisGraph aGraph, int xDim, int yDim) {
        ...
        for (int i = 0; i < (xDim * yDim); i++) {
            ...
            for (Object neighborMove : neighborMoves) {
                // connect current with the possible move that has minimum number of its (possible moves)
                if (!mxGraphStructure.areConnected(aGraph, vertices[i], neighborMove)) {
                    graph.insertEdge(parent, null, getNewEdgeValue(aGraph), vertices[i], neighborMove);
                }
	...
	private Object getNextKnightMove(mxAnalysisGraph aGraph, int xDim, int yDim, int xCoord, int yCoord, ArrayList<Object> resultPath) {
        ...
        for (Object possibleMove : possibleMoves) {
            int currValue = (int) costFunction.getCost(new mxCellState(view, possibleMove, null));
            ...
            if ((currMoveNum < minMoveNum || (currMoveNum == minMoveNum && currDistance > biggestDistance)) && !resultPath.contains(possibleMove)) {
                biggestDistance = currDistance;
                minMoveNum = currMoveNum;
                currVertex = possibleMove;
            }
        }
        return currVertex;
    }
