...
public class mxGraphAnalysis {
     ...
     * edges, starting with <code>from</code> and ending with <code>to</code>.
     *
    ...
    public Object[] getShortestPath(mxGraph graph, Object from, Object to,
        ...
        for (int j = 0; j < steps; j++) {
            ...
            if (e != null) {
                for (Object e1 : e) {
                    Object[] opp = graph.getOpposites(new Object[]{e1}, obj);
                    if (opp != null && opp.length > 0) {
                        ...
                        if (neighbour != null && neighbour != obj
                                && neighbour != from) {
                            double newPrio = prio + ((cf != null) ? cf.getCost(view.getState(e1)) : 1);
                            node = q.getNode(neighbour, true);
                            double oldPrio = node.getKey();
                            if (newPrio < oldPrio) {
                                pred.put(neighbour, e1);
        ...
        ArrayList<Object> list = new ArrayList<>(2 * steps);
        Object obj = to;
    ...
    public Object[] getMinimumSpanningTree(mxGraph graph, Object[] v,
        ...
        ArrayList<Object> mst = new ArrayList<>(v.length);
    ...
    public Object[] getMinimumSpanningTree(mxGraph graph, Object[] v,
        ...
        ArrayList<Object> result = new ArrayList<>(e.length);
        mxCellState[] edgeStates = sort(view.getCellStates(e), cf);

        for (mxCellState edgeState : edgeStates) {
            Object source = edgeState.getVisibleTerminal(true);
            Object target = edgeState.getVisibleTerminal(false);
    ...
    public mxUnionFind getConnectionComponents(mxGraph graph, Object[] v,
        ...
        for (Object e1 : e) {
            mxCellState state = view.getState(e1);
            Object source = (state != null) ? state.getVisibleTerminal(true) : view.getVisibleTerminal(e1, true);
            Object target = (state != null) ? state.getVisibleTerminal(false) : view.getVisibleTerminal(e1, false);
    ...
    public mxCellState[] sort(mxCellState[] states, final mxICostFunction cf) {
        ...
        Collections.sort(result, (mxCellState o1, mxCellState o2) -> {
    ...
    public double sum(mxCellState[] states, mxICostFunction cf) {
        ...
        for (mxCellState state : states) {
            sum += cf.getCost(state);
        }
