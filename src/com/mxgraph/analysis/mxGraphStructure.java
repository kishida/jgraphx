...
public class mxGraphStructure {
    ...
    public static boolean isConnected(mxAnalysisGraph aGraph) {
        ...
        while (queue.size() > 0) {
            ...
            for (Object neighborVertice : neighborVertices) {
                //get the index of the neighbor vertex
                int index = 0;
                for (int k = 0; k < vertexNum; k++) {
                    if (vertices[k].equals(neighborVertice)) {
                        index = k;
                    }
                }
                if (visited[index] == 0) {
	...
	public static void makeSimple(mxAnalysisGraph aGraph) {
        ...
        Set<Set<Object>> vertexSet = new HashSet<>();
        ArrayList<Object> duplicateEdges = new ArrayList<>();

        for (Object currEdge : edges) {
            Object source = aGraph.getTerminal(currEdge, true);
            Object target = aGraph.getTerminal(currEdge, false);
            Set<Object> currSet = new HashSet<>();
            currSet.add(source);
            currSet.add(target);
	...
	public static Object[][] getGraphComponents(mxAnalysisGraph aGraph) {
        ...
        ArrayList<ArrayList<Object>> componentList = new ArrayList<>();
        ArrayList<Object> unvisitedVertexList = new ArrayList<>(Arrays.asList(vertices));
        ...
        while (unvisitedVertexList.size() > 0) {
            ...
            if (!isInComponent) {
                final ArrayList<Object> currVertexList = new ArrayList<>();

                mxTraversal.bfs(aGraph, currVertex, (Object vertex, Object edge) -> {
	...
	public static void makeTreeDirected(mxAnalysisGraph aGraph, Object startVertex) throws StructuralException {
        if (isTree(aGraph)) {
            mxGraphProperties.setDirected(aGraph.getProperties(), false);
            final ArrayList<Object> bFSList = new ArrayList<>();
            mxGraph graph = aGraph.getGraph();
            final mxIGraphModel model = graph.getModel();
            Object parent = graph.getDefaultParent();

            mxTraversal.bfs(aGraph, startVertex, (Object vertex, Object edge) -> {
                bFSList.add(vertex);
                return false;
            });
            ...
            for (int i = 0; i < bFSList.size(); i++) {
                ...
                for (Object currVertex : neighbors) {
	...
	public static void complementaryGraph(mxAnalysisGraph aGraph) {
        ArrayList<ArrayList<mxCell>> oldConnections = new ArrayList<>();
        mxGraph graph = aGraph.getGraph();
        Object parent = graph.getDefaultParent();
        ...
        for (int i = 0; i < vertexCount; i++) {
            ...
            ArrayList<mxCell> neighborVertexes = new ArrayList<>();
        ...
        for (int i = 0; i < vertexCount; i++) {
            ArrayList<mxCell> oldNeighbors = new ArrayList<>();
	...
	public static void setDefaultGraphStyle(mxAnalysisGraph aGraph, boolean resetEdgeValues) {
        ...
        for (Object vertice : vertices) {
            model.setStyle(vertice, basicVertexStyleString);
        }
        ...
        for (Object edge : edges) {
            model.setStyle(edge, edgeString);
        }

        if (resetEdgeValues) {
            for (Object edge : edges) {
                model.setValue(edge, null);
            }
            for (Object edge : edges) {
                model.setValue(edge, aGraph.getGenerator().getNewEdgeValue(aGraph));
            }
        }
    }
	...
	public static Object[] getCutVertices(mxAnalysisGraph aGraph) {
        ArrayList<Object> cutVertexList = new ArrayList<>();
	...
	public static Object[] getCutEdges(mxAnalysisGraph aGraph) {
        ArrayList<Object> cutEdgeList = new ArrayList<>();
	...
	public static Object[] getSourceVertices(mxAnalysisGraph aGraph) throws StructuralException {
        ...
        for (Object currVertex : vertices) {
            Object[] outEdges = aGraph.getEdges(currVertex, null, false, true, true, true);
            Object[] inEdges = aGraph.getEdges(currVertex, null, true, false, true, true);
