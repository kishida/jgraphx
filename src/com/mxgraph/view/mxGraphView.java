...
public class mxGraphView extends mxEventSource {
    ...
    public mxRectangle updateBoundingBox(mxCellState state) {
        ...
        if (mxUtils.getString(style, mxConstants.STYLE_SHAPE, "").equals(
            ...
            if (mxUtils.getString(style, mxConstants.STYLE_IMAGE) != null) {
                ...
                y = switch (imgValign) {
                    case mxConstants.ALIGN_TOP -> state.getY();
                    case mxConstants.ALIGN_BOTTOM -> state.getY() + state.getHeight() - h;
                    default -> state.getY() + (state.getHeight() - h) / 2;
                }; // MIDDLE
    ...
    public void updatePoints(mxCellState edge, List<mxPoint> points,
        ...
        if (edge != null) {
            List<mxPoint> pts = new ArrayList<>();
    ...
    public mxEdgeStyleFunction getEdgeStyle(mxCellState edge,
        ...
        if (edgeStyle instanceof mxEdgeStyleFunction edgeStyleFunction) {
            return edgeStyleFunction;
        }
    ...
    public mxPerimeterFunction getPerimeterFunction(mxCellState state) {
        ...
        if (perimeter instanceof mxPerimeterFunction perimeterFunction) {
     ...
     * @param source Boolean indicating if the next point for the source or
    ...
    public mxCellState[] getCellStates(Object[] cells) {
        List<mxCellState> result = new ArrayList<>(cells.length);
