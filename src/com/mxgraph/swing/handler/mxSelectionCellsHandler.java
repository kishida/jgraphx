...
public class mxSelectionCellsHandler implements MouseListener,
     ...
     *
     */
    protected transient mxIEventListener refreshHandler = (Object source, mxEventObject evt) -> {
        if (isEnabled()) {
            refresh();
        }
    };
     ...
     *
     */
    protected transient PropertyChangeListener labelMoveHandler = (PropertyChangeEvent evt) -> {
    ...
    public mxSelectionCellsHandler(final mxGraphComponent graphComponent) {
        ...
        graphComponent.addListener(mxEvent.PAINT, (Object sender, mxEventObject evt) -> {
    ...
    public void refresh() {
        ...
        for (Object tmp1 : tmp) {
            mxCellState state = graph.getView().getState(tmp1);
            if (state != null && state.getCell() != graph.getView().getCurrentRoot()) {
                mxCellHandler handler = oldHandlers.remove(tmp1);
                if (handler != null) {
