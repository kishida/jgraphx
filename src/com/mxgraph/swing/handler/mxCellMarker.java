 ...
 *
 * public mxCellState process(MouseEvent e) { mxCellState state = null;
 *
 * if (isEnabled()) { state = getState(e); boolean isValid = (state != null) ?
 ...
 * return state; }
 *
 * public void highlight(mxCellState state) { if (validState != state) {
 * Rectangle dirty = null;
 *
 * if (validState != null) { validState.setStyle(lastStyle); dirty =
 ...
 *
 * if (dirty != null) { dirty.add(tmp); } else { dirty = tmp; }
 *
 * dirty.grow(4, 4); }
 *
 * validState = state; graphComponent.repaint(dirty); } }
 *
 * public void reset() { highlight(null); }
 *
